...
@"INSERT INTO dbo.Archivo (CasoID,NombreArchivo,TipoArchivo,DatosArchivo,FechaSubida)
  VALUES (@CasoID,@NombreArchivo,@TipoArchivo,@DatosArchivo,@FechaSubida)", a);
                    ...
                    try { _ = GenerarCorreoCreacionCasoSoporteAsync(id); } catch (Exception ex) { System.Diagnostics.Trace.TraceError("Error al enviar correo async (ID " + id + "): " + ex.Message); }
            ...
            try
            {
                ...
                using (var cn = new SqlConnection(connectionString))
                {
                    ...
                    using (var tx = cn.BeginTransaction())
                    {
                        ...
                        try { await GenerarCorreoCreacionCasoSoporteAsync(casoId); } catch (Exception ex) { System.Diagnostics.Trace.TraceError("Error al enviar correo async (casoId " + casoId + "): " + ex.Message); }
