...
import { ChevronLeft, FileText, Download, Users, Gift, Package, Search } from 'lucide-react';
...
function AsistenciaReport() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const censoRes = await api.get('/attendance/censo', { params: { eventoId: 1, porPagina: 1000 } });
        const allData = censoRes.data.data || [];
        setRows(allData.filter((r: any) => r.Asistio > 0));
      } catch {}
      setLoading(false);
    })();
  }, []);

  if (loading) return <p style={{ textAlign: 'center', color: '#9ca3af' }}>Cargando...</p>;
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        {[
          { label: 'Asistieron', value: rows.length, color: '#065f46', bg: '#d1fae5' },
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
        ...
        <button onClick={() => window.open('/api/reports/entregas.csv?eventoId=1', '_blank')}
          style={{ background: '#10b981', color: 'white', border: 'none', borderRadius: 8, padding: '10px 16px', fontWeight: 600, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              {data.map(r => (
                <tr key={r.entregaId} style={{ borderBottom: '1px solid #f3f4f6' }}>
                  <td style={{ padding: '8px 14px' }}>
                    <div style={{ fontWeight: 600 }}>{r.colaboradorNombre}</div>
                    <div style={{ fontSize: 11, color: '#da121a' }}>{r.colaboradorCarnet}</div>
                  </td>
            ...
            <tbody>
              {items.map((item, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #f3f4f6' }}>
                  ...
                  <td style={{ padding: '8px 14px', textAlign: 'center' }}>
                    <span style={{ fontWeight: 700, color: item.stockActual <= 5 ? '#dc2626' : item.stockActual <= 10 ? '#d97706' : '#10b981' }}>
                      {item.stockActual}
                    </span>
                  </td>
                  ...
                  <td style={{ padding: '8px 14px', textAlign: 'center' }}>
                    <button onClick={() => verDetalle(item.nombreJuguete)}
                      style={{ background: 'none', border: '1px solid #e5e7eb', borderRadius: 6, padding: '4px 8px', cursor: 'pointer', fontSize: 12 }}>
                      👁️ Ver
                    </button>
                  </td>
                </tr>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.4)' }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: 16, padding: 24, width: '90%', maxWidth: 700, margin: 20, maxHeight: '80vh', overflowY: 'auto' }}>
            ...
            {loadingDetalle ? (
              ...
              <p style={{ textAlign: 'center', color: '#9ca3af' }}>Sin entregas registradas para este juguete</p>
            )}
          </div>
        </div>
      )}
